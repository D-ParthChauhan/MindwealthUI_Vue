      ...
      <table class="tbl">
        ...
        <tbody>
          ...
          >
            ...
            <td class="runic-body" :style="{ fontSize: '10px', fontWeight: '600', color: row.current_color ?? 'var(--t2)' }" v-html="row.current" />
...
const { nightly, variables } = useRunicMacro()

function showVixBypass(row: RunicVariableRow) {
  return row.name === 'VIX' && (nightly.value?.vix_bypass_active || row.vix_bypass)
}
</script>
